...
class LanguageEncoder(nn.Module):  #i means channel-independent
    def __init__(self, c_in, patch_num, patch_len, max_seq_len=1024,
        ...
        # Text
        self.language_knowledge = torch.load('./layers/text_embedding.pt')
        self.text_linear = nn.Linear(4096,d_model)
        self.ts2text = CrossEncoder(d_model, n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm, attn_dropout=attn_dropout, dropout=dropout,
                                   pre_norm=pre_norm, activation=act, res_attention=res_attention, n_layers=n_layers, store_attn=store_attn)
        self.text2ts = CrossEncoder(d_model, n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm, attn_dropout=attn_dropout, dropout=dropout,
                                   pre_norm=pre_norm, activation=act, res_attention=res_attention, n_layers=n_layers, store_attn=store_attn)

        
    def forward(self, x) -> Tensor:                                              # x: [bs x nvars x d_model x patch_num]
        bs,n_vars = x.shape[0],x.shape[1]
 
        expanded_tensor = self.language_knowledge.unsqueeze(0)
        # 
        language_knowledge = expanded_tensor.repeat(bs*n_vars, 1, 1).to(x.device)           # language_knowledge [bs*nvars, text_num, 4096]
        language_knowledge  = self.text_linear(language_knowledge)              # language_knowledge [bs*nvars, text_num, d_model]
        language_knowledge = self.dropout(language_knowledge) 
        # Input encoding
        x = x.permute(0,1,3,2)                                                   # x: [bs x nvars x patch_num x d_model ]
        x = torch.reshape(x, (x.shape[0]*x.shape[1],x.shape[2],x.shape[3]))      # u: [bs * nvars x patch_num x d_model]

        #
        language_knowledge =  self.ts2text(language_knowledge,x,x)               # language_knowledge: [bs * nvars x patch_num x d_model]
        z = self.text2ts(x,language_knowledge,language_knowledge)
